...
The `/analyze` response should include at least `word_count`, `unique_word_count` (count of distinct alphabetic words, case-insensitive), `sentence_count`, `key_terms` (a list), and `estimated_tokens`. The `/evaluate` response needs at least `score` (0–100 float) and `feedback` (a string explaining how well they did).
